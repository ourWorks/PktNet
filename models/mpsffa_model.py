...
class MPSFFA(nn.Module):
    def __init__(self, is_print=False, num_classes=2):
        super(MPSFFA, self).__init__()
        self.backbone = BackBone(is_print)

        # 修改1：为了使得num_classes适应二分类和三分类任务
        # patch_num = np.prod((3, 3, 3)) # 下面的计算，一定记得要转换为整数！！！
        # self.fc = nn.Sequential(nn.Linear(64 * 4 * 5 * 4, 1024),
        self.fc = nn.Sequential(nn.Linear(64 * 27, 1024),
                                nn.ReLU(),
                                nn.Linear(1024, 512),
                                nn.ReLU(),
                                nn.Linear(512, num_classes))
        # self.fea_weighted = BlockScore(64, 4 * 5 * 4)
        self.fea_weighted = BlockScore(64, 27)
