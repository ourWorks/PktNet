...
class BaseConvExtraLayer(nn.Module):
    def __init__(self):
        ...
        # 3D卷积层
        self.conv1 = nn.Sequential(nn.Conv3d(1, 64, kernel_size=7, padding=1, stride=2))
        self.baseConv = nn.Sequential(nn.Conv3d(64, 512, kernel_size=3, padding=1), nn.MaxPool3d(8))
...
class BaseModel_PacketLayer(nn.Module):

    def __init__(self, num_classes):
        super(BaseModel_PacketLayer, self).__init__()
